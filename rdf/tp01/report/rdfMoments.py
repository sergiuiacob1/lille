...
def rdfMomentCentre(img, p, q):
    ...
    x = np.array([(x-cx) ** p for x in range(1, img.shape[0] + 1)])
    y = np.array([(y-cy) ** q for y in range(1, img.shape[1] + 1)])
...
def calculateMainImageAxis(nom, normalise=True):
...
def calculateImageMoment(nom, p, q, normalise=True):
    img = rdfReadGreyImage(nom)
    plt.imshow(img)
    plt.show()

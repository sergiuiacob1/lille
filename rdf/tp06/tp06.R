...
# Plot lines and points again
plot(grille)
zp<-assigne_test$post[,3]-pmax(assigne_test$post[,2],assigne_test$post[,1])
contour(xp1,xp2,matrix(zp,50),add=TRUE,levels=0,drawlabels=FALSE)
zp<-assigne_test$post[,2]-pmax(assigne_test$post[,3],assigne_test$post[,1])
contour(xp1,xp2,matrix(zp,50),add=TRUE,levels=0,drawlabels=FALSE)

...
for filename in ['a.txt', 'b.txt', 'c.txt', 'e.txt', 'f.txt', 'd.txt']:
    ...
    def get_best_library_for_signup(current_day):
        ...
        # lib = random.choice(list(choices))
        # no =  ((D - current_day) - libraries[lib].signup) * libraries[lib].limit
        # return lib, libraries[lib].books[:no]
    ...
    for i in range(0, D):
        ...
        libraries_available = len(libraries_already_signed)
        scanned = set()
        for x in books_to_scan:
            ...
            if libraries_available == 0:
                break

        for x in scanned:
            books_to_scan.remove(x)

    # remove libraries that did not scan anything
    for i in range(0, L):
        if len(libraries_scan_order[i]) == 0 and i in libraries_already_signed:
            libraries_already_signed.remove(i)

    filename_output = filename.split('.txt')[0] + '_out.txt'
    with open(filename_output, 'w') as f:
        f.write(f'{len(libraries_already_signed)}\n')
        for x in libraries_already_signed:
